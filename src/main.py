...
input_file_nr: int = int(input())
if input_file_nr == 1:
    expr, var_count = expr_from_table("../resources/table.txt")
...
rpn: list[str] = reverse_polish_notation(expr)
...
gates: list[Gate] = setup_canvas(canvas, var_count, WIDTH)

# construirea listei de porti logice cu specifice
gates: list[Gate] = build_gates(rpn, gates)
